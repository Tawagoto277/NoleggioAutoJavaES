...
public class EliminaCliente extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			PreparedStatement stmt = DB.getPrepareStatement("SELECT COUNT(*) AS count FROM noleggi WHERE pt_noleggi_cliente = ?");
			stmt.setInt(1, id);
			...
			if(count == 0) {
				stmt = DB.getPrepareStatement("DELETE FROM clienti WHERE ID_CLIENTE = ?");			
				stmt.setInt(1, id);
