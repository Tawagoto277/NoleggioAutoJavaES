 ...
 */
@WebServlet("/inserisciStati")
public class InserisciStati extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InserisciStati() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String nome = request.getParameter("nomeStato");
		int inserimento = 0;
		...
		if(nome.trim().length() == 0 || nome == null) {
			...
			try {
				PreparedStatement stmt = DB.getPrepareStatement("INSERT INTO stati (nome) VALUE(?)");
